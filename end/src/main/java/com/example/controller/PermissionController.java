...
public class PermissionController {
    ...
    public Result<List<Permission>> tree() {
        ...
        return Result.success(createTree(0L ,list));
    }
     ...
     * 递归生成菜单树
     */
    private List<Permission> createTree(Long parent, List<Permission> menus) {
        List<Permission> treeMenu = new ArrayList<>();
        for (Permission permission : menus) {
            if (permission.getParent() != null && parent.equals(permission.getParent())) {
                treeMenu.add(permission);
                permission.setChildren(createTree(permission.getId(), menus));
            }
        }
        return treeMenu;
    }
