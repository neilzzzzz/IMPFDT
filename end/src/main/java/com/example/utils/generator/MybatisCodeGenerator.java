...
public class MybatisCodeGenerator {
    ...
    private static final String schemaName = "realestate";   // 数据库名称，必填
    private static final String[] table = {"progress", ""};   // 必填，第一个是表名，第二个是实体类的名字
    private static final String modelName = "进度";   // 必填
    ...
    static void createVue(String entityName, String tableName) throws SQLException {
        ...
        for (TableColumn tableColumn : tableColumns) {
            ...
            if (tableColumn.getColumnName().endsWith("img")) {
                tableColumnBuilder.append(space6 + "<el-table-column label=\"图片\"><template slot-scope=\"scope\"><el-image style=\"width: 100px; height: 100px\" :src=\"scope.row.img\" :preview-src-list=\"[scope.row.img]\"></el-image></template></el-table-column>\n");
            } else {
            ...
            StringBuilder formBuilder = formItemBuilder.append(space8 + "<el-form-item label=\"" + tableColumn.getColumnComment() + "\" label-width=\"120px\">\n");
